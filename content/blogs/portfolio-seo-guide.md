...
Search `site:yourdomain.com`. If nothing returns, submit the site through Google Search Console and check that robots.txt is not blocking crawlers.
...
Check `site:yourdomain.com` in Google right now. If nothing returns, submit the site to Search Console and check that your host or builder is not blocking crawlers by default — that single issue makes every other optimisation irrelevant.

Then win your own name. Use your full name identically across your site, LinkedIn and GitHub, put it in the homepage title tag and H1, link all three profiles to each other, and buy the domain in your name if it is available.

Finally, fix LinkedIn before the website. Most recruiter sourcing happens inside LinkedIn's own database rather than through search engines, so that profile is doing more work than your portfolio — and if you want inbound beyond name searches, start writing about your specialism, because a gallery of projects gives search engines almost nothing to index.
