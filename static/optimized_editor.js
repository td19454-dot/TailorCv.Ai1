...
`;
    ...
    /* "See what changed" lives in static/changes_modal.js, shared with the
       Chrome extension. Loaded before this file (optimized_editor.html). */
    function showChangesModal(payload) {
        if (!window.TCVChangesModal) {
            console.error("[TailorCV] changes_modal.js did not load.");
            return;
        }
        window.TCVChangesModal.open(payload, currentHtml);
    }
