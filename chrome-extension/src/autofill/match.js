...
const COUNTRIES = new Set([
  'india', 'united states', 'united states of america', 'usa', 'us', 'canada',
  'united kingdom', 'uk', 'england', 'scotland', 'wales', 'ireland', 'australia',
  'new zealand', 'germany', 'france', 'spain', 'italy', 'netherlands', 'belgium',
  'sweden', 'norway', 'denmark', 'finland', 'poland', 'portugal', 'switzerland',
  'austria', 'singapore', 'japan', 'china', 'south korea', 'brazil', 'mexico',
  'argentina', 'chile', 'south africa', 'nigeria', 'kenya', 'egypt', 'israel',
  'uae', 'united arab emirates', 'saudi arabia', 'qatar', 'pakistan',
  'bangladesh', 'sri lanka', 'nepal', 'philippines', 'indonesia', 'malaysia',
  'thailand', 'vietnam', 'remote',
]);

const US_STATE_CODES = new Set(Object.values(US_STATES));

/**
 * Split a free-text location ("Kolkata, West Bengal, India") into parts.
 *
 * Resume location strings are one line of prose, and UserApplyProfile stores
 * exactly one such string, so this is the only source for a form that wants
 * City and Country in separate boxes. Anything it cannot identify is left
 * EMPTY rather than guessed — an empty field is surfaced to the user, whereas
 * a wrong city is submitted silently.
 */
export function splitLocation(location) {
  const out = { city: '', state: '', country: '', postalCode: '' };
  ...
  // Country first. It is the most reliably identifiable part (a closed set),
  // and removing it exposes the postal code, which commonly sits on the part
  // BEFORE it ("San Francisco, CA 94107, USA") rather than at the very end.
  const tail = parts[parts.length - 1];
  if (COUNTRIES.has(tail.toLowerCase())) {
    out.country = tail;
    parts = parts.slice(0, -1);
  }
  if (!parts.length) return out;

  // Now a postal code, wherever it ended up trailing.
  const lastIdx = parts.length - 1;
  const zip = parts[lastIdx].match(/\b(\d{5}(?:-\d{4})?|[A-Z]{1,2}\d[A-Z\d]? ?\d[A-Z]{2}|[A-Z]\d[A-Z] ?\d[A-Z]\d|\d{6})\b/i);
  if (zip) {
    out.postalCode = zip[1];
    const stripped = parts[lastIdx].replace(zip[0], '').trim().replace(/[,\s]+$/, '');
    if (stripped) parts[lastIdx] = stripped;
    else parts = parts.slice(0, -1);
  }
  if (!parts.length) return out;

  // Whatever is last of two-or-more parts is the region. Done for any region,
  // not just recognised US states: "Kolkata, West Bengal" means the same thing
  // structurally as "Austin, TX", and treating an unrecognised one as part of
  // the city name put "Kolkata, West Bengal" in a City box.
  if (parts.length >= 2) {
...
/** The two-letter code for a US state name, or '' — forms often want the code. */
export function usStateCode(state) {
  const s = String(state == null ? '' : state).trim();
  if (!s) return '';
  if (US_STATE_CODES.has(s.toUpperCase())) return s.toUpperCase();
